...
std::unique_ptr<QBert::QBertState> QBert::QBertState::OnNotify(dae::Event event, dae::Subject*)
{
	...
	{
		return std::make_unique<StunnedQBertState>(m_qbert, m_pConnSpriteComp, m_pMoveComp, m_pConnLevel);
	}
	if (event.id == dae::make_sdbm_hash("LEVEL_COMPLETED"))
	{
		return std::make_unique<FrozenQBertState>(m_qbert, m_pConnSpriteComp, m_pMoveComp, m_pConnLevel);
	}
