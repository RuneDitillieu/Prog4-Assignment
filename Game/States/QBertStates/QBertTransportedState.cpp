...
QBert::TransportedQBertState::TransportedQBertState(dae::GameObject* qbert, dae::SpriteComp* spriteComp, QBert::QBertMoveComp* moveComp, LevelBase* level)
...
void QBert::TransportedQBertState::OnEnter()
{
	...
	dae::ServiceLocator::GetSoundSystem().Play(dae::SoundId(QBert::Sound::Lift), 0.5f);
}
...
std::unique_ptr<QBert::QBertState> QBert::TransportedQBertState::OnNotify(dae::Event event, dae::Subject*)
{
	...
	{
		return std::make_unique<QBert::DroppingQBertState>(m_qbert, m_pConnSpriteComp, m_pMoveComp, m_pConnLevel);
	}
